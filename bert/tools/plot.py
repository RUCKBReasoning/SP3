...
def draw_figure1(p_config, name):
    ...
    ax.set_title('Intermediate dimensions of each layer in PLM')
...
def draw_figure2(p_config, name):
    ...
    ax.set_ylabel('percentage (%)')
    ax.set_title('Hidden dimensions of each layer in PLM')
...
def draw_figure3(p_config, name):
    ...
    ax.set_ylabel('percentage (%)')
    ax.set_title('Number of attention heads of each layer in PLM')
    ax.set_xticks(x, x_values)
    ax.legend(loc='upper right', ncols=3)
    ax.set_ylim(0, 100)
