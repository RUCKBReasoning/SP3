...
class PackedBertLayerConfig:

    def __init__(self,
            input_dim: int = 768,
            attn_output_dim: int = 768,
            ffn_output_dim: int = 768,
            num_heads: int = 12,
            qk_dim: int = 768,
            vo_dim: int = 768,
            ffn_dim: int = 3072,
        ...
        self.ffn_output_dim = ffn_output_dim
...
class PackedBertConfig(BertConfig):
    
    def __init__(self,
            per_layer_config: Optional[List[PackedBertLayerConfig]] = None,
            **kwargs
    ):
        super().__init__(**kwargs)
        if per_layer_config is None:
            per_layer_config = [PackedBertLayerConfig() for _ in range(self.num_hidden_layers)]
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, PackedBertEncoder):
...
class PackedBertEmbeddings(nn.Module):
...
class PackedBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_scores = attention_scores / math.sqrt(self.origin_head_size)
        ...
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_vo_dim,)        
...
class PackedBertSelfOutput(nn.Module):
    def __init__(self, config, layer_config: PackedBertLayerConfig):
        super().__init__()
        self.residual = nn.Linear(layer_config.input_dim, layer_config.attn_output_dim)
        self.dense = nn.Linear(layer_config.vo_dim, layer_config.attn_output_dim)
...
class PackedBertAttention(nn.Module):
    def __init__(self, config, layer_config: PackedBertLayerConfig, position_embedding_type=None):
        super().__init__()
        self.self = PackedBertSelfAttention(
            config,
            layer_config,
            position_embedding_type=position_embedding_type
        )
        self.output = PackedBertSelfOutput(config, layer_config)
...
class PackedBertIntermediate(nn.Module):
    def __init__(self, config, layer_config: PackedBertLayerConfig):
        super().__init__()
        self.dense = nn.Linear(layer_config.attn_output_dim, layer_config.ffn_dim)
        if isinstance(config.hidden_act, str):
...
class PackedBertOutput(nn.Module):
    def __init__(self, config, layer_config: PackedBertLayerConfig):
        super().__init__()
        self.residual = nn.Linear(
            layer_config.attn_output_dim, layer_config.ffn_output_dim)
        self.dense = nn.Linear(
            layer_config.ffn_dim, layer_config.ffn_output_dim)
        self.LayerNorm = PackedLayerNorm(
            config.hidden_size,
            layer_config.ffn_output_dim,
            eps=config.layer_norm_eps
...
class PackedBertLayer(nn.Module):
    def __init__(self, config, layer_config: PackedBertLayerConfig):
        super().__init__()
...
class PackedBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList(
            [PackedBertLayer(config, config.per_layer_config[i])
...
class PackedBertPooler(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        input_hidden_size = config.per_layer_config[-1].ffn_output_dim
        self.dense = nn.Linear(input_hidden_size, config.hidden_size)
...
class PackedBertModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = PackedBertEmbeddings(config)
        self.encoder = PackedBertEncoder(config)
...
class PackedBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config

        self.bert = PackedBertModel(config)
        classifier_dropout = (
...
class PackedBertForQuestionAnswering(BertPreTrainedModel):
