...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, CompactBertEncoder):
...
class CompactBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = LayerNormWithCompactor(embedding_size, eps=config.layer_norm_eps)
...
class CompactBertSelfAttention(nn.Module):
...
class CompactBertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = CompactBertSelfAttention(
            config,
            position_embedding_type=position_embedding_type
        )
        self.output = CompactBertSelfOutput(config)
...
class CompactBertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class CompactBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = CompactBertAttention(config)
        self.is_decoder = config.is_decoder
...
class CompactBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer: List[CompactBertLayer] = nn.ModuleList([CompactBertLayer(config) for _ in range(config.num_hidden_layers)])
...
class CompactBertModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = CompactBertEmbeddings(config)
        self.encoder = CompactBertEncoder(config)

        if add_pooling_layer:
            self.pooler = CompactBertPooler(config)
        else:
            self.pooler = None
    ...
    def frozen_params(self):
        ...
        for name, module in self.named_modules():
            ...
            ) or name.endswith('intermediate.dense'):
...
class CompactBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config

        self.bert = CompactBertModel(config)
        classifier_dropout = (
    ...
    def post_init(self):
        ...
        with torch.no_grad():
            ...
            for module in self.modules():
                if isinstance(module, Compactor):
...
class CompactBertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = CompactBertModel(config, add_pooling_layer=False)
